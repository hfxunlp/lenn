...
for t=1, firstcycle do
	if psg then
		...
		if not (o[-1]:narrow(1, 1, eplens):equal(stdo1) and o[-1]:narrow(1, eplens+1, plens+lens-eplens):equal(torch.zeros(plens+lens-eplens)) and o:narrow(1,1,bsize-1):narrow(2, 1, lens):equal(stdo) and o:narrow(1,1,bsize-1):narrow(2, lens+1, plens):equal(torch.zeros(bsize-1, plens))) then
			psg=false
			print("forward error")
			print(stdo)
			print(stdo1)
		...
		if not (gi[-1]:narrow(1, 1, eplens):equal(stdgi1) and gi[-1]:narrow(1, eplens+1, plens+lens-eplens):equal(torch.zeros(plens+lens-eplens)) and gi:narrow(1,1,bsize-1):narrow(2, 1, lens):equal(stdgi) and gi:narrow(1,1,bsize-1):narrow(2, lens+1, plens):equal(torch.zeros(bsize-1, plens))) then
			psg=false
